...
def get_area_across_slices(key_body_n,str_pat_id,s_body):
    CT_path = get_info_replanned(str_pat_id,1)
        ...
        if str_pat_id in existing_patients:
            ...
            for key_body_n in range(0,len(key_bodies_to_save)):
                ...
                if key_bodies_to_save[key_body_n]=='BODY':
                    ...
                    h2,k2 = get_center2(path_k,str_pat_id)
                    body_sim = get_equal_body_fov(contour_body,h2,k2,r)
                    gc.collect()
            ...
            for key_body_n in range(0,len(contours_neck)):
                ...
                print(min(c_body[:,2]),max(c_body[:,2]))
                print(min(contours00[:,2]),max(contours00[:,2]))
              #  print(z_min2,z_max2)
                params = []
               
                vol_neck = get_volumes_from_contours(c_body,pixel_spacing2)
                surface_area_neck  = get_surface_area(c_body,pixel_spacing2)
