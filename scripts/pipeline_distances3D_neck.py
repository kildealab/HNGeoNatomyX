...
def pipeline_distances_neck(param_name='distances3Dneck', path_contours,CSV_patients_ids,path_CBCTs):
    t_init = process_time()
    ...
    for str_pat_id in ids_patients:
        ...
        if str_pat_id in existing_patients:
            ...
            for key_body_n in range(0,len(contours_neck)):
                ...
                dmax = get_max_dist_body(pv.PolyData(trim_neck),pv.PolyData(trim_neck0))
                chamfer = get_chamfer(trim_neck,trim_neck0)
...
if __name__ == "__main__":
    ...
    pipeline_distances_neck(path_contours,CSV_patients_ids,path_CBCTs)
