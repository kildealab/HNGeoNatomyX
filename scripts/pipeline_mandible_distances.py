...
def get_patient_data(path_src, patient_num):  #Obtiene los datos de los pacientes en el director
...
def get_center(body1,body2,r):
    ...
    k1 = y1[1] - np.sqrt((r*0.5)**2 - (max_x-h)**2)
    k2 = y2[1] - np.sqrt((r*0.5)**2 - (min_x-h)**2)
    ...
    k3 = y3[1] - np.sqrt((r*0.5)**2 - (max_x/2-h)**2)
    k4 = y4[1] - np.sqrt((r*0.5)**2 - (min_x/2-h)**2)
   
    k6 = y6[1] - np.sqrt((r*0.5)**2 - (max_x*7/8-h)**2)
    k7 = y7[1] - np.sqrt((r*0.5)**2 - (min_x*7/8-h)**2)
    ...
    for str_pat_id in ['630']:
        ...
        if str_pat_id in existing_patients:
            ...
            if len(body_list)==0:
                ...
                mandiblee = get_key_mandible(str_pat_id,path_rs)
                #mandiblee = get_key_mandible(str_pat_id)
                mR= rtdsm.get_pointcloud(mandiblee, path_rs, False)[0]
            else:
                ...
                mandiblee = get_key_mandible(str_pat_id,path_rs_b0)
                mR= rtdsm.get_pointcloud(mandiblee, path_rs_b0, False)[0]
            ...
            mand = get_surface_marching_cubes(mR,pixel_spacing2)
            ...
            for key_body_n in range(0,len(key_bodies_to_save)):
                ...
                dist_points = get_distances_of_points_from_cloud(pv.PolyData(mand),pv.PolyData(contours[key_body_n]))
