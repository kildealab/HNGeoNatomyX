...
#TO TRIM THE NECK WE NEED TO TAKE A COMMON AREA, IN THIS CASE WAS -18 MM - 0 MM (Z RANGE)
#THUS THE Z_MIN = -18 AND Z_MAX = 0. HERE THE '+ SPACING' DOES NOT ANY CHANGE BECAUSE THE THRESHOLDS IN THE LISTS BELLOW ARE 
# '>' AND NOT '>=' 
...
PATH_DEST = '120_patients/neck_distances/'
    ...
    for str_pat_id in ids_news:
        ...
        if str_pat_id in existing_patients:
            ...
            for key_body_n in range(0,len(contours_neck)):
                ...
                #print(min(c_trim_body[:,2]),max(c_trim_body[:,2]))
                #print(min(c_trim_body2[:,2]),max(c_trim_body2[:,2]))
                #print(z_min2,z_max2)
                params = []
                ...
                gc.collect()
                d_max = get_max_dist_body(pv.PolyData(c_trim_body),pv.PolyData(c_trim_body2))
                chamber = get_chamber(c_trim_body,c_trim_body2)
