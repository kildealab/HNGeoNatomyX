...
def get_volumes_from_contours(contours_body, IMG_RES,smooth_iter=0):
    # ================================================================================
    #contours_body, contours_PTV = trim_contours_to_match_zs(contours_body, contours_PTV,z_min,z_max)
...
def get_surface_area(contours_body,IMG_RES):
    ...
    s_area = surface_body.area #*IMG_RES[0]*IMG_RES[1]
    return s_area
...
def get_area_across_slices(key_body_n,str_pat_id,s_body):
    CT_path = get_info_replanned(str_pat_id,0)
                
    start_x, start_y, start_z, pixel_spacing = get_start_position_dcm(CT_path)
    s_body1 = get_surface_marching_cubes(s_body,pixel_spacing[0:2])
    ...
    for z in zs:
      ...
      area = get_area(slice1,start_x,start_y,pixel_spacing)
      areas.append(area)
...
import csv
import os
PATH_DEST = '120_patients/neck_volume_areav2_pxv2/'
    ...
    for str_pat_id in ids_news:
        ...
        if str_pat_id in existing_patients:
            ...
            if len(body_list)==0:
                ...
                for body in bodies_sorted_rs:
                    ...
                    if len(body_contour)==0:
                        ...
                        if body not in key_bodies_to_save:
                            ...
                            #mandiblee
                            mR= rtdsm.get_pointcloud(key_mandible, path_rs, False)[0]
                ...
                mR= rtdsm.get_pointcloud(key_mandible, path_rs_b0, False)[0]
            ...
            for key_body_n in range(0,len(key_bodies_to_save)):
                ...
                if key_bodies_to_save[key_body_n]=='BODY':
                    ...
                    trim_body,trim_body2 = trim_contours_to_match_zs_neck(body_sim.points,body_sim.points,-18,0)
                    ...
                    trim_body,trim_body2 = trim_contours_to_match_zs_neck(contour_body,contour_body,-18,0)
                    contours_neck.append(trim_body)           
            ...
            for key_body_n in range(0,len(contours_neck)):
                ...
                vol_neck = get_volumes_from_contours(c_body,pixel_spacing2[0:2])
                surface_area_neck  = get_surface_area(c_body,pixel_spacing2[0:2])
                
                compactness = surface_area_neck**3/vol_neck**2
                mean_area = get_area_across_slices(key_body_n,str_pat_id,c_body)
