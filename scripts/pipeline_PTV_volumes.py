...
def get_start_position_dcm(CT_path):
    #positions = []
    ff = [file for file in os.listdir(CT_path) if 'CT' in file]
    d = pydicom.dcmread(CT_path+'/'+ff[0])
...
def pipeline_volumes(param_name='volumesPTV',path_k = '/mnt/iDriveShare/OdetteR/Registration_and_contours/Contours/'):
    ...
    for str_pat_id in ['630']:
        ...
        if str_pat_id in existing_patients:
            ...
            #print(sorted([d for d in os.listdir(patient_path) if d[0:4] == 'Body']))
            ptv = []
            if len(body_list)==0:
                ...
                #ptv_key = get_keys_v2('ptv',str_pat_id, path_rs_b0)
            ...
            pixel_spacing2 = get_start_position_dcm(CT_path)
             
            for key_body_n in range(0,len(key_bodies_to_save)):
                print(pixel_spacing2)
                params = []
                ...
                vol_body, vol_PTV_inner, vol_PTV_outer = get_volumes_from_contours(contours_PTV, b1,pixel_spacing2)
                print('\tProcess time of parameters (' + key_bodies_to_save[key_body_n] + '): ' + str(process_time()-t1) + ' s')
          
                print('vol_PTV_inner',vol_PTV_inner)
                print('vol_PTV_outer',vol_PTV_outer)
